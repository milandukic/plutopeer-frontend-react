...
const WheelComponent = ({
  ...
  console.log("wheel winningSegment:", winningSegment);
  useEffect(() => {
    wheelInit();
    if (isStart) spin();
  }, [isStart]);
  ...
  const initCanvas = () => {
    ...
    //  spin();
    let spinImage = document.getElementsByClassName("winnerFixedImage");
    spinImage[0].addEventListener("click", handleClick, false);
  ...
  const drawWheel = () => {
    ...
    ctx.beginPath();
    ctx.arc(centerX, centerY, 90, 0, PI2, false);
    ctx.closePath();
  ...
  const drawNeedle = () => {
    ...
    ctx.beginPath();
    ctx.moveTo(centerX + 20, centerY - 90);
